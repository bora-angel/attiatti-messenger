...
if(isset($_POST['join_submit'])){
    ...
    $atti_id = $_POST['join_id'];
    $atti_pw = $_POST['join_pw'];
    ...
    $join_atti = "insert into `atti_user` values('','".$atti_id."',
                                                PASSWORD('".$atti_pw."'),
                                                '".$atti_name."',
                                                '".$atti_telnum."',
    ...
    if(!$result_join){
        echo '<script>alert("자료추가실패"); location="./first.php"; </script>';
        echo mysqli_error($result_join);

    }else{
        echo '<script>alert("성공적으로 추가 되었습니다."); location="./first.php";</script>';
    }
