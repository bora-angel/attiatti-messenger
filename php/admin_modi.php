    ...
    <section class="desk_top">
        ...
        <article class="join_window" id="join_window">
            ...
            <div class="join_grey">
                ...
                <hr class="join_line">
                <form action="../php/admin_modify_ok.php" method="post" class="join_form" enctype="multipart/form-data">
...
<?php
                ...
                $sub_sql = "select * from `atti_user` where user_id ='".$modi_id."' AND user_telnum = '".$atti_num."' ";
                $tb_result = mysqli_query($atti_connect, $sub_sql);
                if($atti_subdb =mysqli_fetch_assoc($tb_result)){

                    echo '<input type="hidden" name="modify_flag" value="'.$modi_flag.'">';
                    echo '<input type="hidden" name="modify_id" value="'.$modi_id.'">';
                    echo '<input type="hidden" name="modify_name" value="'.$modi_name.'">';
                    echo '<input type="hidden" name="modify_telnum" value="'.$atti_num.'">';
                    echo '<label class="comm_label">이름<input type="text" name="join_name" class="input_join" value="'.$atti_subdb['user_name'].'" tabindex=8> </label>';
                    echo '<label class="comm_label">전화번호
                        <input type="text" name="join_telnum" class="input_join" value="'.$atti_subdb['user_telnum'].'" tabindex=10></label>';
                    echo '<label class="comm_label gender_label">성별
                        <select name="join_gender" class="input_join gender_join" value="'.$atti_subdb['user_gender'].'" tabindex=13>
                    ...
                    echo '<label class="comm_label birth_label">생년월일
                        <input type="date" name="join_birth" class="input_join" value="'.$atti_subdb['user_birth'].'" tabindex=17></label>';

                    echo '<label class="comm_label">이메일
                        <input type="text" name="join_email" class="input_join" value="'.$atti_subdb['user_email'].'" tabindex=20></label>';
                    echo '<label class="comm_label address_label">주소
                        <input type="text" name="join_address" class="input_join" value="'.$atti_subdb['user_address'].'" tabindex=25></label>';
                    echo '<hr class="form_line">
                    <label class="joinmemo_label">메모
                        <input type="text" name="join_memo" class="input_join memo_join" value="'.$atti_subdb['user_etc'].'" tabindex=40>
                    </label >
                    <hr class="form_line">
                    ...
                    echo '<div class="pic_right">
                        ...
                        <label class="face_pic">
                            <input type="file" accept="image/*" name="join_file" value="'.$atti_subdb['user_photo'].'"class="file_join" tabindex=30>
                        </label>';
                }else{
                    $sub_sql = "select * from `atti_user_telbook` where tb_user_id ='".$modi_id."' AND tb_name = '".$modi_name."' ";
                    $tb_result = mysqli_query($atti_connect, $sub_sql);
