...
<?
    ...
    if(mysqli_num_rows($admin_result)){
        ...
        while($atti_dbrow = mysqli_fetch_assoc($user_result)){
            echo '<form action="./admin_info.php" id="hidden_num'.$i.'" method="post">';
            echo '<input type="hidden" id="sub_hidden'.$i.'" name="sub_hidden" value="'.$atti_dbrow['user_telnum'].'" >';
            echo '<input type="hidden" name="sub_hidden_name" value="'.$atti_dbrow['user_name'].'" >';
            echo '<li class="list_people" id="user_mem'.$i.'">'.$atti_dbrow['user_name'].'('.$atti_dbrow['user_telnum'].')</li></form>';
            $j = 1;
            ...
            #윗줄에 해당하는 유저db의 아이디와 패스워드가 부합하는 tb의 목록
                if($atti_indb['tb_user_id'] == $atti_dbrow['user_id'] && $atti_indb['tb_user_pw']==$atti_dbrow['user_pw']){
                    echo '<form action="./admin_info.php" id ="in_hidden'.$j.'" method="POST"><ul class="in_list"><li class="comm_li" id="intb_mem'.$j.'">';
                    echo '<input type="hidden" id="insub_hidden'.$j.'" name="insub_hidden" value="'.$atti_dbrow['user_id'].'" >';
                    echo '<input type="hidden" name="sub_hidden_telnum" value="'.$atti_indb['tb_telnum'].'" >';
    ...
    setInterval("real_time()",50);
        ...
        this.addEventListener("click",(e)=>{
            ...
            switch(e.target.id){
                    ...
                    case "intb_mem"+tmp_num:
                    function tb_func(){
                        document.getElementById("in_hidden"+tmp_num).submit();
                    }
                    return tb_func();
                    break;
                    ...
                    case "help_btn":
                        alert("본 홈페이지는 포트폴리오용으로 제작되었습니다. ");
                    break;
                }
